...
local path_compiled_files =
    string.format('%s%s', vim.fn.stdpath 'cache', '/awesomedark')
...
function M.load()
    ...
    if not vim.loop.fs_stat(dark_compiled) then
        local palette = require 'awesomedark.palette'
        local theme = require('awesomedark.themes').dark(palette, M.config)
    ...
    if not vim.loop.fs_stat(light_compiled) then
        local palette = require 'awesomedark.palette'
        local theme = require('awesomedark.themes').light(palette, M.config)
    ...
    local f = assert(
        ...
        '[awesomedark.nvim] could not load cache'
    )
    f()
end
...
local h=vim.api.nvim_set_hl]],
    ...
    local hgs = require('awesomedark.highlight_groups').get(config, theme)
    ...
    if not f then
        local err_path = (path_sep == '/' and '/tmp' or os.getenv 'TMP')
            .. '/awesomedark_error.lua'
        print(
            string.format(
                '[awesomedark.nvim] error, open %s for debugging',
                err_path
            )
        )
    ...
    local err_msg = string.format(
        '[awesomedark.nvim] permission denied while writing compiled file to %s%s%s',
        path_compiled_files,
        path_sep,
        filename_compiled_colorscheme
    )
...
vim.api.nvim_create_user_command('VSCodeModernCompile', function()
    ...
    local dark_theme = require('awesomedark.themes').dark(palette, M.config)
    M.compile(M.config, dark_theme, 'dark')

    local light_theme = require('awesomedark.themes').light(palette, M.config)
    M.compile(M.config, light_theme, 'light')

    vim.notify '[awesomedark.nvim] colorscheme compiled'
